...
const formFor = (
  ...
  const builder : FormBuilder = {
    input: (name, opts = {}) => {
      ...
      if (opts.as === 'textarea') {
        fields.push(`<textarea cols="${opts.cols ?? 20}" rows="${opts.rows ?? 40}" name="${name}">${value}</textarea>`);
      } else {
        fields.push(`<input name="${name}" type="text" value="${value}"${opts.class === undefined ? "" : ` class="${opts.class}"`}>`);
      }
    },
    submit: (value = 'Save') => {
